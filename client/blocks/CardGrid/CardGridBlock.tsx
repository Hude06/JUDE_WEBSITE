      ...
      <nav className="mx-auto max-w-[var(--container-wide)] px-6 md:px-10 py-6 md:py-8">
        <ul className="border-t border-[color:var(--color-hairline-strong)]">
          {block.cards.map((card, i) => {
            ...
            return (
              <li key={card.title} className="border-b border-[color:var(--color-hairline)]">
                {card.link ? (
                  <a
                    href={card.link}
        ...
        {block.cards.map((card) => {
          const inner = (
            <article className="group">
              ...
              <p className="mt-2 text-base text-[color:var(--color-muted)] leading-relaxed">{card.description}</p>
            </article>
          );
